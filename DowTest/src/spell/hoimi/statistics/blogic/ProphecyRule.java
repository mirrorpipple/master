...
//            db.commit();
    ...
    public void Prophet(Connection db, ProphecyInput input, ResultSet rs, int funds, BigDecimal err){
        ...
        //###############################
        //コード表
        //1:利益売り（山谷）　2:損切り（low値参照）　3:利益売り（マージン超え）　4:損切り（谷の閾値割り）
        //###############################
        ...
        BigDecimal escape = BigDecimal.valueOf(0.03);
        boolean stanbyFlg = false;
        try {
            while(rs.next()){
                cnt ++;
                ...
                if (!input.isSetupFlg()) {
                    ...
                    input.setBetHigh(rs.getBigDecimal("STARTRATE"));
                }


                //絶対値による利確定チェック
                if(input.isBetFlg() && (input.getThisHigh().subtract(input.getBetStart())).compareTo(margin) >=0){
                    input.setBetFlg(false);
                    BetTheGame(db, input, 3, margin);
                    stanbyFlg = true;
                }
                
                //絶対値による損切りチェック
                if(input.isBetFlg() && (input.getThisLow().subtract(input.getBetHigh())).compareTo(margin) >=0){
                    input.setBetFlg(false);
                    BetTheGame(db, input, 3, margin);
                    stanbyFlg = true;
                }
                ...
                if (input.getThisHigh().compareTo((input.getBfHigh().add(err))) >= 0){
                    ...
                    if (input.getThisTrend() != input.getBfTrend()){
                        ...
                        if (input.getBfHigh().compareTo((input.getVllyHigh())) >= 0){
                            ...
                            if (!stanbyFlg && !input.isBetFlg()){
                                //賭けていなかったとき。ここで賭ける。
                                //値を持たせて別methodで実行させたいときに、setter,getter役に立つのか。
                                input.setBetFlg(true);
                                input.setBetStart(input.getThisHigh());
                                input.setBetHigh(input.getBfHigh());
                                BetTheGame(db, input, 0, null);
                            }
                    ...
                    //まず始めに損切りチェック
                    if(input.isBetFlg() && input.getBetHigh().compareTo(input.getThisHigh()) >=0){
                        //下降の閾値超え、かつ賭けていた場合は無条件で損切り
                            input.setBetFlg(false);
                            BetTheGame(db, input, 2, null);
                    }
                        //前回トレンドと比較し、凹凸をチェックする。
                        if (input.getThisTrend() != input.getBfTrend()){
                            //山ができたとき、前山Mtの値と比較する。
    ...
    public void BetTheGame(Connection db, ProphecyInput input, int tacticsCode, BigDecimal margin) throws SQLException{
      ...
      //#############################SQL設定##############################
        ...
        if(input.isBetFlg()){
            ...
            if(tacticsCode == 3){
                ...
                calcs = calcs.multiply(input.getBetStart().add(margin));
                ps.setBigDecimal(4,input.getThisHigh());
